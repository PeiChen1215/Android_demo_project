...
v0.1.0 - 销售与图片支持（草稿） - 2025-12-22
- 新增：销售/收银（POS）模块脚手架（`SaleActivity`、`SaleDAO`、`Sale`/`SaleLine` 模型）。
- 新增：商品行项显示适配器与布局（`SaleLineAdapter` / `item_sale_line.xml`）。
- 改进：商品详情增加缩略图输入与防抖实时预览（Glide）。
- 持久化：为 `products` 表添加 `thumb_url` 列并在 DAO 中读写该字段。

注意：该发布为功能性草稿，包含基础保存/展示逻辑。建议合并到 `main` 前在测试环境完成以下工作：
- 补充收银流程（结账后减少库存并写库存事务）；
- 增加收据打印/导出与异常回滚处理；
- 为关键流程添加自动化测试；
- 对外发布前清理开发用示例与调试代码。
